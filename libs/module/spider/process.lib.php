 ...
 * @param   string $update      计数器的更新值，如果是'init'，计数器则被初始化为0
 * @return int                      返回计数
 */
function mp_counter($countername, $update = 1) {
    ...
    $cur_path = "{$process_file_dir}btime_{$sysuptime}_ptime_{$ppuptime}";
 ...
 */
function multi_process($num, $stat = true) {
    ...
    while (true) {
        ...
        if ($task_finish == false && $child < $num) {
            ...
            if ($pid) {
                ...
                echo "{$colors['reverse']}{$colors['green']}[+]New Process Forked: {$pid}{$colors['end']}\n";
...
function final_stat() {
    ...
    echo " Efficiency: Process({$colors['reverse']}{$process_num}{$colors['end']}{$colors['green']}),Second({$fetch_rps}),";
    echo "Hour({$fetch_rph}),Day({$colors['reverse']}{$fetch_rpd}{$colors['end']}{$colors['green']}).\n";
...
function signal_handler($signal) {
    ...
    if (array_key_exists($signal, $signals)) {
        kill_all_child();
        echo "\n{$colors['cyan']}Ctrl + C caught,quit!{$colors['end']}\n";
        final_stat();
        exit();
    }
}
...
function mp_msg() {
    ...
    if ($t_lines <= 1) {
        ...
        echo "{$colors['reverse']}{$colors['purple']}";
